...
function registerticketfreshdesk(yourdomain,api_key)
{
      ...
      if ($('#tipo').val() == "Incidente" && $('#subject').val() == "Aplicativo"){
          ...
          description = "Rol: "+$('#tipocustomer').val()+'<br>'+"Sistema operativo: "+$('#so').val()+'<br>'+"Modelo: "+$('#model').val()+'<br>'+"Version del SO: "+$('#versionso').val()+'<br><br><br>'+"Descripcion: "+CKEDITOR.instances['description'].getData()+'<br><br><br>'+"Nombre Completo: "+$('#name').val()+' '+$('#lastname').val()+'<br>'+"Numero Telefonico: "+' '+$('#prefpais').val()+' '+$('#telefono').val()+'<br>'+"Correo Electronico: "+$('#email').val()+"<br>"+"Pais: "+$('#groups_id').val()+"<br>"+"Ciudad: "+$('#cityid').val()+"<br>"+"Usuario: "+$('#user').val();
     }else if ($('#tipo').val() == "Queja" || $('#tipo').val() == "Reclamo" && $('#subject').val() == "Aplicativo"){
          subject = $('#subject').val()+' - '+$('#ctryname').val();
          description = CKEDITOR.instances['description'].getData()+'<br><br><br>'+"Nombre Completo: "+$('#name').val()+' '+$('#lastname').val()+'<br>'+"Numero Telefonico: "+' '+$('#prefpais').val()+' '+$('#telefono').val()+'<br>'+"Correo Electronico: "+$('#email').val()+"<br>"+"Pais: "+$('#groups_id').val()+"<br>"+"Ciudad: "+$('#cityid').val()+"<br>"+"Usuario: "+$('#user').val()+"<br>"+"ID del viaje: "+$('#idtravel').val();
     } else {
          subject = $('#subject').val()+' - '+$('#ctryname').val();
          description = CKEDITOR.instances['description'].getData()+'<br><br><br>'+"Nombre Completo: "+$('#name').val()+' '+$('#lastname').val()+'<br>'+"Numero Telefonico: "+' '+$('#prefpais').val()+' '+$('#telefono').val()+'<br>'+"Correo Electronico: "+$('#email').val()+"<br>"+"Pais: "+$('#groups_id').val()+"<br>"+"Ciudad: "+$('#cityid').val()+"<br>"+"Usuario: "+$('#user').val();
      }
...
var desc = CKEDITOR.instances['description'].getData();
  if ($("#user").val() == ""){
    ...
    alertify.alert('Ingrese su nombre').setHeader('<h3 style="color: orange; font-weight: bold;">Alerta</h3>');
    $(".namevalidate").html('El campo nombre es requerido').css("color", "#FF0000");
    $('#name').focus();

  }else if ($("#lastname").val() == ""){
    alertify.alert('Ingrese su apellido').setHeader('<h3 style="color: orange; font-weight: bold;">Alerta</h3>');
    $(".lastnamevalidate").html('El campo apellido es requerido').css("color", "#FF0000");
    $('#lastname').focus();

  }else if ($("#email").val() == ""){
    alertify.alert('Ingrese su correo electronico').setHeader('<h3 style="color: orange; font-weight: bold;">Alerta</h3>');
    $(".emailvalidate").html('El campo correo electronico es requerido').css("color", "#FF0000");
    $('#email').focus();

  }else if ($("#telefono").val() == ""){
    alertify.alert('Ingrese su telefono').setHeader('<h3 style="color: orange; font-weight: bold;">Alerta</h3>');
    $(".telefonovalidate").html('El campo telefono es requerido').css("color", "#FF0000");
    $('#telefono').focus();

  }else if ($("#telefono").val() != "" && $("#telefono").val().length > 29){
    alertify.alert('Ingrese su telefono correctamente').setHeader('<h3 style="color: orange; font-weight: bold;">Alerta</h3>');
    $(".telefonovalidate").html('El campo telefono es incorrecto').css("color", "#FF0000");
    $('#telefono').focus();
